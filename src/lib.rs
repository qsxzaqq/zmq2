...
impl SocketType {
    fn to_raw(self) -> c_int {
        let raw = match self {
            PAIR => zmq2_sys::ZMQ_PAIR,
            PUB => zmq2_sys::ZMQ_PUB,
            SUB => zmq2_sys::ZMQ_SUB,
            REQ => zmq2_sys::ZMQ_REQ,
            REP => zmq2_sys::ZMQ_REP,
            DEALER => zmq2_sys::ZMQ_DEALER,
            ROUTER => zmq2_sys::ZMQ_ROUTER,
            PULL => zmq2_sys::ZMQ_PULL,
            PUSH => zmq2_sys::ZMQ_PUSH,
            XPUB => zmq2_sys::ZMQ_XPUB,
            XSUB => zmq2_sys::ZMQ_XSUB,
            STREAM => zmq2_sys::ZMQ_STREAM,
        };
        raw as c_int
    }
    fn from_raw(raw: c_int) -> SocketType {
        match raw as u32 {
            zmq2_sys::ZMQ_PAIR => PAIR,
            zmq2_sys::ZMQ_PUB => PUB,
            zmq2_sys::ZMQ_SUB => SUB,
            zmq2_sys::ZMQ_REQ => REQ,
            zmq2_sys::ZMQ_REP => REP,
            zmq2_sys::ZMQ_DEALER => DEALER,
            zmq2_sys::ZMQ_ROUTER => ROUTER,
            zmq2_sys::ZMQ_PULL => PULL,
            zmq2_sys::ZMQ_PUSH => PUSH,
            zmq2_sys::ZMQ_XPUB => XPUB,
            zmq2_sys::ZMQ_XSUB => XSUB,
            zmq2_sys::ZMQ_STREAM => STREAM,
...
pub enum SocketEvent {
    ...
    CONNECTED = zmq2_sys::ZMQ_EVENT_CONNECTED as isize,
    CONNECT_DELAYED = zmq2_sys::ZMQ_EVENT_CONNECT_DELAYED as isize,
    CONNECT_RETRIED = zmq2_sys::ZMQ_EVENT_CONNECT_RETRIED as isize,
    LISTENING = zmq2_sys::ZMQ_EVENT_LISTENING as isize,
    BIND_FAILED = zmq2_sys::ZMQ_EVENT_BIND_FAILED as isize,
    ACCEPTED = zmq2_sys::ZMQ_EVENT_ACCEPTED as isize,
    ACCEPT_FAILED = zmq2_sys::ZMQ_EVENT_ACCEPT_FAILED as isize,
    CLOSED = zmq2_sys::ZMQ_EVENT_CLOSED as isize,
    CLOSE_FAILED = zmq2_sys::ZMQ_EVENT_CLOSE_FAILED as isize,
    DISCONNECTED = zmq2_sys::ZMQ_EVENT_DISCONNECTED as isize,
    MONITOR_STOPPED = zmq2_sys::ZMQ_EVENT_MONITOR_STOPPED as isize,
    HANDSHAKE_FAILED_NO_DETAIL = zmq2_sys::ZMQ_EVENT_HANDSHAKE_FAILED_NO_DETAIL as isize,
    HANDSHAKE_SUCCEEDED = zmq2_sys::ZMQ_EVENT_HANDSHAKE_SUCCEEDED as isize,
    HANDSHAKE_FAILED_PROTOCOL = zmq2_sys::ZMQ_EVENT_HANDSHAKE_FAILED_PROTOCOL as isize,
    HANDSHAKE_FAILED_AUTH = zmq2_sys::ZMQ_EVENT_HANDSHAKE_FAILED_AUTH as isize,
    ALL = zmq2_sys::ZMQ_EVENT_ALL as isize,
}
...
impl SocketEvent {
    ...
    pub fn from_raw(raw: u16) -> SocketEvent {
        use SocketEvent::*;
        match u32::from(raw) {
            zmq2_sys::ZMQ_EVENT_CONNECTED => CONNECTED,
            zmq2_sys::ZMQ_EVENT_CONNECT_DELAYED => CONNECT_DELAYED,
            zmq2_sys::ZMQ_EVENT_CONNECT_RETRIED => CONNECT_RETRIED,
            zmq2_sys::ZMQ_EVENT_LISTENING => LISTENING,
            zmq2_sys::ZMQ_EVENT_BIND_FAILED => BIND_FAILED,
            zmq2_sys::ZMQ_EVENT_ACCEPTED => ACCEPTED,
            zmq2_sys::ZMQ_EVENT_ACCEPT_FAILED => ACCEPT_FAILED,
            zmq2_sys::ZMQ_EVENT_CLOSED => CLOSED,
            zmq2_sys::ZMQ_EVENT_CLOSE_FAILED => CLOSE_FAILED,
            zmq2_sys::ZMQ_EVENT_DISCONNECTED => DISCONNECTED,
            zmq2_sys::ZMQ_EVENT_MONITOR_STOPPED => MONITOR_STOPPED,
            zmq2_sys::ZMQ_EVENT_HANDSHAKE_FAILED_NO_DETAIL => HANDSHAKE_FAILED_NO_DETAIL,
            zmq2_sys::ZMQ_EVENT_HANDSHAKE_SUCCEEDED => HANDSHAKE_SUCCEEDED,
            zmq2_sys::ZMQ_EVENT_HANDSHAKE_FAILED_PROTOCOL => HANDSHAKE_FAILED_PROTOCOL,
            zmq2_sys::ZMQ_EVENT_HANDSHAKE_FAILED_AUTH => HANDSHAKE_FAILED_AUTH,
            zmq2_sys::ZMQ_EVENT_ALL => ALL,
...
pub static DONTWAIT: i32 = zmq2_sys::ZMQ_DONTWAIT as i32;
/// Flag for socket `send` methods that specifies that more frames of a
/// multipart message will follow.
pub static SNDMORE: i32 = zmq2_sys::ZMQ_SNDMORE as i32;
...
impl Error {
    ...
    pub fn from_raw(raw: i32) -> Error {
        match raw {
            ...
            // TODO: these are present in `zmq2-sys`, but not handled, as that
    ...
    pub fn message(self) -> &'static str {
        unsafe {
            let s = zmq2_sys::zmq_strerror(self.to_raw());
...
fn errno_to_error() -> Error {
    Error::from_raw(unsafe { zmq2_sys::zmq_errno() })
}
...
pub fn version() -> (i32, i32, i32) {
    ...
    unsafe {
        zmq2_sys::zmq_version(&mut major, &mut minor, &mut patch);
    }
...
impl RawContext {
    fn term(&self) -> Result<()> {
        zmq_try!(unsafe { zmq2_sys::zmq_ctx_term(self.ctx) });
        Ok(())
    }
}
...
impl Context {
    ...
    pub fn new() -> Context {
        Context {
            raw: Arc::new(RawContext {
                ctx: unsafe { zmq2_sys::zmq_ctx_new() },
            }),
        }
    }
    ...
    pub fn get_io_threads(&self) -> Result<i32> {
        let rc =
            zmq_try!(unsafe { zmq2_sys::zmq_ctx_get(self.raw.ctx, zmq2_sys::ZMQ_IO_THREADS as _) });
        Ok(rc as i32)
    }
    ...
    pub fn set_io_threads(&self, value: i32) -> Result<()> {
        zmq_try!(unsafe {
            zmq2_sys::zmq_ctx_set(self.raw.ctx, zmq2_sys::ZMQ_IO_THREADS as _, value as i32)
        });
        Ok(())
    }
    ...
    pub fn socket(&self, socket_type: SocketType) -> Result<Socket> {
        let sock = unsafe { zmq2_sys::zmq_socket(self.raw.ctx, socket_type.to_raw()) };
...
impl Drop for Socket {
    fn drop(&mut self) {
        if self.owned && unsafe { zmq2_sys::zmq_close(self.sock) } == -1 {
    ...
    ( $(#[$meta:meta])*
      pub $getter:ident => $constant_name:ident as $ty:ty
        ...
        pub fn $getter(&self) -> Result<$ty> {
            <$ty as sockopt::Getter>::get(self.sock, zmq2_sys::$constant_name as c_int)
        }
    };
}
    ...
    ( $(#[$meta:meta])*
      pub $setter:ident => $constant_name:ident as $ty:ty
        ...
        pub fn $setter(&self, value: $ty) -> Result<()> {
            <$ty as sockopt::Setter>::set(self.sock, zmq2_sys::$constant_name as c_int, value)
        }
    };
}
    ...
    T: Into<Message>,
{
    fn send(self, socket: &Socket, flags: i32) -> Result<()> {
        ...
        zmq_try!(unsafe { zmq2_sys::zmq_msg_send(msg_ptr(&mut msg), socket.sock, flags as c_int) });
        Ok(())
    }
}
...
impl Socket {
    ...
    pub fn bind(&self, endpoint: &str) -> Result<()> {
        ...
        zmq_try!(unsafe { zmq2_sys::zmq_bind(self.sock, c_str.as_ptr()) });
        Ok(())
    }
    ...
    pub fn unbind(&self, endpoint: &str) -> Result<()> {
        let c_str = ffi::CString::new(endpoint.as_bytes()).unwrap();
        zmq_try!(unsafe { zmq2_sys::zmq_unbind(self.sock, c_str.as_ptr()) });
        Ok(())
    }
    ...
    pub fn connect(&self, endpoint: &str) -> Result<()> {
        let c_str = ffi::CString::new(endpoint.as_bytes()).unwrap();
        zmq_try!(unsafe { zmq2_sys::zmq_connect(self.sock, c_str.as_ptr()) });
        Ok(())
    }
    ...
    pub fn disconnect(&self, endpoint: &str) -> Result<()> {
        let c_str = ffi::CString::new(endpoint.as_bytes()).unwrap();
        zmq_try!(unsafe { zmq2_sys::zmq_disconnect(self.sock, c_str.as_ptr()) });
        Ok(())
    }
    ...
    pub fn monitor(&self, monitor_endpoint: &str, events: i32) -> Result<()> {
        ...
        zmq_try!(unsafe {
            zmq2_sys::zmq_socket_monitor(self.sock, c_str.as_ptr(), events as c_int)
        });
        Ok(())
    }
    ...
    pub fn recv(&self, msg: &mut Message, flags: i32) -> Result<()> {
        zmq_try!(unsafe { zmq2_sys::zmq_msg_recv(msg_ptr(msg), self.sock, flags as c_int) });
        Ok(())
    }
    ...
    pub fn recv_into(&self, bytes: &mut [u8], flags: i32) -> Result<usize> {
        ...
        let rc = zmq_try!(unsafe {
            zmq2_sys::zmq_recv(self.sock, bytes_ptr, bytes.len(), flags as c_int)
        });
        Ok(rc as usize)
    }
    ...
    pub fn get_socket_type(&self) -> Result<SocketType> {
        sockopt::get(self.sock, zmq2_sys::ZMQ_TYPE as c_int).map(SocketType::from_raw)
    }

    /// Return true if there are more frames of a multipart message to receive.
    pub fn get_rcvmore(&self) -> Result<bool> {
        sockopt::get(self.sock, zmq2_sys::ZMQ_RCVMORE as c_int).map(|o: i64| o == 1i64)
    }
    ...
    pub fn get_identity(&self) -> Result<Vec<u8>> {
        ...
        sockopt::get_bytes(self.sock, zmq2_sys::ZMQ_ROUTING_ID as c_int, 255)
    }
    ...
    pub fn get_socks_proxy(&self) -> Result<result::Result<String, Vec<u8>>> {
        ...
        sockopt::get_string(self.sock, zmq2_sys::ZMQ_SOCKS_PROXY as c_int, 255, true)
    }

    pub fn get_mechanism(&self) -> Result<Mechanism> {
        sockopt::get(self.sock, zmq2_sys::ZMQ_MECHANISM as c_int).map(|mech| match mech {
            zmq2_sys::ZMQ_NULL => Mechanism::ZMQ_NULL,
            zmq2_sys::ZMQ_PLAIN => Mechanism::ZMQ_PLAIN,
            zmq2_sys::ZMQ_CURVE => Mechanism::ZMQ_CURVE,
            zmq2_sys::ZMQ_GSSAPI => Mechanism::ZMQ_GSSAPI,
    ...
    pub fn get_plain_username(&self) -> Result<result::Result<String, Vec<u8>>> {
        // 255 = arbitrary size
        sockopt::get_string(self.sock, zmq2_sys::ZMQ_PLAIN_USERNAME as c_int, 255, true)
    }

    pub fn get_plain_password(&self) -> Result<result::Result<String, Vec<u8>>> {
        // 256 = arbitrary size based on std crypto key size
        sockopt::get_string(self.sock, zmq2_sys::ZMQ_PLAIN_PASSWORD as c_int, 256, true)
    }

    pub fn get_zap_domain(&self) -> Result<result::Result<String, Vec<u8>>> {
        // 255 = arbitrary size
        sockopt::get_string(self.sock, zmq2_sys::ZMQ_ZAP_DOMAIN as c_int, 255, true)
    }
    ...
    pub fn get_last_endpoint(&self) -> Result<result::Result<String, Vec<u8>>> {
        ...
        sockopt::get_string(
            self.sock,
            zmq2_sys::ZMQ_LAST_ENDPOINT as c_int,
            256 + 9 + 1,
            true,
        )
    }
    ...
    pub fn get_curve_publickey(&self) -> Result<Vec<u8>> {
        sockopt::get_bytes(self.sock, zmq2_sys::ZMQ_CURVE_PUBLICKEY as c_int, 32)
    }
    ...
    pub fn get_curve_secretkey(&self) -> Result<Vec<u8>> {
        sockopt::get_bytes(self.sock, zmq2_sys::ZMQ_CURVE_SECRETKEY as c_int, 32)
    }
    ...
    pub fn get_curve_serverkey(&self) -> Result<Vec<u8>> {
        ...
        sockopt::get_bytes(self.sock, zmq2_sys::ZMQ_CURVE_SERVERKEY as c_int, 32)
    }
    ...
    pub fn get_gssapi_principal(&self) -> Result<result::Result<String, Vec<u8>>> {
        ...
        sockopt::get_string(
            self.sock,
            zmq2_sys::ZMQ_GSSAPI_PRINCIPAL as c_int,
            260,
            true,
        )
    }
    ...
    pub fn get_gssapi_service_principal(&self) -> Result<result::Result<String, Vec<u8>>> {
        ...
        sockopt::get_string(
            self.sock,
            zmq2_sys::ZMQ_GSSAPI_SERVICE_PRINCIPAL as c_int,
            260,
            true,
        )
    }
...
pub fn poll(items: &mut [PollItem], timeout: i64) -> Result<i32> {
    let rc = zmq_try!(unsafe {
        zmq2_sys::zmq_poll(
            items.as_mut_ptr() as *mut zmq2_sys::zmq_pollitem_t,
            items.len() as c_int,
            timeout as c_long,
        )
    });
...
pub fn proxy(frontend: &Socket, backend: &Socket) -> Result<()> {
    zmq_try!(unsafe { zmq2_sys::zmq_proxy(frontend.sock, backend.sock, ptr::null_mut()) });
    Ok(())
}
...
pub fn proxy_with_capture(
    ...
    zmq_try!(unsafe { zmq2_sys::zmq_proxy(frontend.sock, backend.sock, capture.sock) });
    Ok(())
}
...
pub fn proxy_steerable(
    ...
    zmq_try!(unsafe {
        zmq2_sys::zmq_proxy_steerable(frontend.sock, backend.sock, ptr::null_mut(), control.sock)
    });
    Ok(())
}
...
pub fn proxy_steerable_with_capture(
    ...
    zmq_try!(unsafe {
        zmq2_sys::zmq_proxy_steerable(frontend.sock, backend.sock, capture.sock, control.sock)
    });
    Ok(())
}
...
pub fn has(capability: &str) -> Option<bool> {
    ...
    unsafe { Some(zmq2_sys::zmq_has(c_str.as_ptr()) == 1) }
}
...
pub fn z85_decode(data: &str) -> result::Result<Vec<u8>, DecodeError> {
    ...
    unsafe {
        zmq2_sys::zmq_z85_decode(dest.as_mut_ptr(), c_str.into_raw());
    }
