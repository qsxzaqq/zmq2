...
pub struct Message {
    msg: zmq2_sys::zmq_msg_t,
}

impl Drop for Message {
    fn drop(&mut self) {
        unsafe {
            let rc = zmq2_sys::zmq_msg_close(&mut self.msg);
            assert_eq!(rc, 0);
        }
    }
}
...
impl Message {
    ...
    where
        F: FnOnce(&mut zmq2_sys::zmq_msg_t) -> i32,
    {
        let mut msg = zmq2_sys::zmq_msg_t::default();
        let rc = f(&mut msg);
        if rc == -1 {
    ...
    pub fn new() -> Message {
        unsafe { Self::alloc(|msg| zmq2_sys::zmq_msg_init(msg)) }
    }
    ...
    pub unsafe fn with_capacity_unallocated(len: usize) -> Message {
        Self::alloc(|msg| zmq2_sys::zmq_msg_init_size(msg, len as size_t))
    }
    ...
    pub fn get_more(&self) -> bool {
        let rc = unsafe { zmq2_sys::zmq_msg_more(&self.msg) };
        rc != 0
    }
    ...
    pub fn gets<'a>(&'a mut self, property: &str) -> Option<&'a str> {
        ...
        let value = unsafe { zmq2_sys::zmq_msg_gets(&self.msg, c_str.as_ptr()) };
...
impl Deref for Message {
    ...
    fn deref(&self) -> &[u8] {
        ...
        unsafe {
            ...
            let data = zmq2_sys::zmq_msg_data(ptr);
            let len = zmq2_sys::zmq_msg_size(ptr) as usize;
...
impl DerefMut for Message {
    fn deref_mut(&mut self) -> &mut [u8] {
        ...
        // this message.
        unsafe {
            let data = zmq2_sys::zmq_msg_data(&mut self.msg);
            let len = zmq2_sys::zmq_msg_size(&self.msg) as usize;
...
pub fn msg_ptr(msg: &mut Message) -> *mut zmq2_sys::zmq_msg_t {
    &mut msg.msg
}
