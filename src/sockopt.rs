...
pub fn get_bytes(sock: *mut c_void, opt: c_int, size: size_t) -> Result<Vec<u8>> {
    ...
    zmq_try!(unsafe {
        zmq2_sys::zmq_getsockopt(sock, opt, value.as_mut_ptr() as *mut c_void, &mut size)
    });
    value.truncate(size);
    Ok(value)
}
...
fn setsockopt_null(sock: *mut c_void, opt: c_int) -> Result<()> {
    zmq_try!(unsafe { zmq2_sys::zmq_setsockopt(sock, opt, ptr::null(), 0) });
    Ok(())
}
